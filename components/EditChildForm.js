...
import {
    ...
    KeyboardAvoidingView, Image, StyleSheet
} from 'react-native';
...
export default class EditChild extends React.Component {
    ...
    uploadImage(child) {
        ...
        // formData.append('file', { uri: imageUri, type: 'image/jpg'});
        ...
        myHeaders.append("Host", 'rest-service.azurewebsites.net')
        var formdata = new FormData();
        formdata.append("file", imageUri, "img.jpg");
        ...
        fetch("https://rest-service.azurewebsites.net/api/v1/upload-image/636", requestOptions)
            .then(response => response.text())
            .then((result) => { console.log(result); this.setState({ successDisplay: true }); })
            .catch(error => console.log('error', error));
        // RNFetchBlob.fetch('PUT', photoUrl, {
        //     'Content-Type': 'multipart/form-data',
        //   }, [ 
        //       { name: 'file', filename: 'photo.jpg', type: 'image/png', data: RNFetchBlob.wrap(imageUri) }
        //     ]) .then((resp) => {
        //         this.setState({ successDisplay: true });
